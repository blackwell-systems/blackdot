...
import (
	...
	"github.com/blackwell-systems/blackdot/v4/internal/template"
...
func runTemplateEdit(cmd *cobra.Command, args []string) error {
	...
	if _, err := os.Stat(localFile); os.IsNotExist(err) {
		...
		fmt.Println("Run 'blackdot template init' first")
		return err
	}
	...
	fmt.Println()
	Info("Run 'blackdot template render' to apply changes")
	return nil
}
...
#
# Generated by: blackdot template init
# Generated on: %s
#
# Edit this file to customize your configuration.
# Run 'blackdot template render' after making changes.
# ============================================================
...
func runTemplateLink(cmd *cobra.Command, args []string) error {
	...
	if linked == 0 {
		Info("No files to link (run 'blackdot template render' first)")
	} else {
		fmt.Printf("\nLinked %d file(s)\n", linked)
	}
