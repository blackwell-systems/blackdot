...
# Unified location handling across backends
# ============================================================

# Get the configured vault location
# Returns JSON: {"type": "folder", "value": "dotfiles"} or empty
vault_get_location() {
    local vault_items_file="${1:-$HOME/.config/dotfiles/vault-items.json}"
    ...
    jq -c '.vault_location // empty' "$vault_items_file" 2>/dev/null
}

# Get location type (folder, vault, directory, etc.)
vault_get_location_type() {
    local vault_items_file="${1:-$HOME/.config/dotfiles/vault-items.json}"
    local location
    location=$(vault_get_location "$vault_items_file")

    if [[ -n "$location" ]]; then
        echo "$location" | jq -r '.type // "none"'
    else
        echo "none"
    fi
}

# Get location value (the folder name, vault name, etc.)
vault_get_location_value() {
    local vault_items_file="${1:-$HOME/.config/dotfiles/vault-items.json}"
    local location
    location=$(vault_get_location "$vault_items_file")

    if [[ -n "$location" ]]; then
        echo "$location" | jq -r '.value // empty'
    fi
}

# Set vault location in config
# Usage: vault_set_location "folder" "dotfiles"
vault_set_location() {
    local loc_type="$1"
    local loc_value="$2"
    local vault_items_file="${3:-$HOME/.config/dotfiles/vault-items.json}"

    # Ensure directory exists
    mkdir -p "$(dirname "$vault_items_file")"

    # Create or update the file
    if [[ -f "$vault_items_file" ]]; then
        # Update existing file
        local tmp_file="${vault_items_file}.tmp"
        jq --arg type "$loc_type" --arg value "$loc_value" \
            '.vault_location = {type: $type, value: $value}' \
            "$vault_items_file" > "$tmp_file" && mv "$tmp_file" "$vault_items_file"
    else
        # Create new file with just location
        cat > "$vault_items_file" <<EOF
{
  "\$schema": "https://json-schema.org/draft/2020-12/schema",
  "\$comment": "Generated by dotfiles vault setup",
  "vault_location": {
    "type": "$loc_type",
    "value": "$loc_value"
  },
...
# List available locations from backend
# Usage: vault_list_locations "$SESSION"
vault_list_locations() {
    _ensure_backend_loaded || return 1

    if type vault_backend_list_locations >/dev/null 2>&1; then
        vault_backend_list_locations "$@"
    else
        warn "Backend $(vault_name) does not support location listing"
        echo "[]"
    fi
}

# List items in a specific location
# Usage: vault_list_items_in_location "folder" "dotfiles" "$SESSION"
vault_list_items_in_location() {
    _ensure_backend_loaded || return 1

    if type vault_backend_list_items_in_location >/dev/null 2>&1; then
        vault_backend_list_items_in_location "$@"
    else
        # Fallback: list all items (legacy behavior)
        vault_backend_list_items "${@: -1}"  # Last arg is session
    fi
}

# Create a new location (folder/vault/directory)
# Usage: vault_create_location "dotfiles" "$SESSION"
vault_create_location() {
    _ensure_backend_loaded || return 1

    if type vault_backend_create_location >/dev/null 2>&1; then
        vault_backend_create_location "$@"
    else
        warn "Backend $(vault_name) does not support location creation"
        return 1
    fi
}

# Check if a location exists
# Usage: vault_location_exists "dotfiles" "$SESSION"
vault_location_exists() {
    _ensure_backend_loaded || return 1

    if type vault_backend_location_exists >/dev/null 2>&1; then
        vault_backend_location_exists "$@"
    else
        # Fallback: try to list and check
        local name="$1"
        local session="$2"
        vault_list_locations "$session" | jq -e ".[] | select(. == \"$name\")" >/dev/null 2>&1
    fi
}
