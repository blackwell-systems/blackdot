...
- **SSH `keys` subcommands** — `blackdot tools ssh keys` is now a parent command with `list`, `load`, `unload`, and `clear` subcommands. Both `ssh keys unload <key>` and `ssh unload <key>` work. Called without a subcommand it still lists keys.
