...
- **Cross-platform binary resolution for shared filesystems** — Shell init (`_blackdot_resolve_bin`) now prefers platform-specific binaries (e.g. `blackdot-linux-amd64`, `blackdot-darwin-arm64`) before falling back to the generic `bin/blackdot`. Fixes `exec format error` when a macOS host shares its dotfiles directory with a Lima or NFS-mounted Linux VM. The `_blackdot_go_bin()` helper and `blackdot-upgrade` also use the same platform-aware resolution. Rebuild hints now include the correct `GOOS`/`GOARCH` for the current platform.
...
- **Vault Shell Scripts** - Deleted 20 shell scripts and backend implementations (~6,000 lines) now that Go has full parity:
  - 16 vault operations scripts: `vault/*.sh` (restore-*.sh, sync-*.sh, create-*.sh, delete-*.sh, etc.)
  - 3 backend implementations: `vault/backends/*.sh` (bitwarden.sh, 1password.sh, pass.sh)
  - 1 interface specification: `vault/backends/_interface.md`
